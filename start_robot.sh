...
# 自动从安装路径发现可用配置
INFERENCE_CFG_DIR="$CFG_BASE"

# 选择推理配置
INFERENCE_CONFIGS=()
if [ -d "$INFERENCE_CFG_DIR" ]; then
    while IFS= read -r f; do INFERENCE_CONFIGS+=("$f"); done < <(ls "$INFERENCE_CFG_DIR"/inference*.yaml 2>/dev/null | xargs -n1 basename)
fi
...
# 选择 robot 配置
ROBOT_CFG_DIR="$CFG_BASE"
ROBOT_CONFIGS=()
if [ -d "$ROBOT_CFG_DIR" ]; then
    while IFS= read -r f; do ROBOT_CONFIGS+=("$f"); done < <(ls "$ROBOT_CFG_DIR"/robot*.yaml 2>/dev/null | xargs -n1 basename)
fi
ROBOT_CONFIGS+=("自定义路径")
